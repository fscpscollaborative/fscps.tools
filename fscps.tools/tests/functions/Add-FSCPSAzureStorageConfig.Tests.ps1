Describe "Add-FSCPSAzureStorageConfig Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Add-FSCPSAzureStorageConfig).ParameterSets.Name | Should -Be 'AccessToken', 'SAS'
		}
		
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['Name']
		...
		It 'Should have the expected parameter AccountId' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['AccountId']
		...
		It 'Should have the expected parameter AccessToken' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['AccessToken']
		...
		It 'Should have the expected parameter SAS' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['SAS']
		...
		It 'Should have the expected parameter Container' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['Container']
		...
		It 'Should have the expected parameter Temporary' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['Temporary']
		...
		It 'Should have the expected parameter Force' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['Force']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Add-FSCPSAzureStorageConfig).Parameters['ProgressAction']
	...
	Describe "Testing parameterset AccessToken" {
		...
		AccessToken -Name -AccountId -AccessToken -Container -Temporary -Force -ProgressAction
		#>
	}
 	Describe "Testing parameterset SAS" {
		<#
		SAS -Name -AccountId -SAS -Container
		SAS -Name -AccountId -SAS -Container -Temporary -Force -ProgressAction
		#>
	}
