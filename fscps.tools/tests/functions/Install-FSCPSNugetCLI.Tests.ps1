Describe "Install-FSCPSNugetCLI Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Install-FSCPSNugetCLI).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Install-FSCPSNugetCLI).Parameters['Path']
		...
		It 'Should have the expected parameter Url' {
			$parameter = (Get-Command Install-FSCPSNugetCLI).Parameters['Url']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Install-FSCPSNugetCLI).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -
		__AllParameterSets -Path -Url -ProgressAction
		#>
	}
