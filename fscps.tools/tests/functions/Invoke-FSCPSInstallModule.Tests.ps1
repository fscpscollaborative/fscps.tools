Describe "Invoke-FSCPSInstallModule Unit Tests" -Tag "Unit" {
	...
	Describe "Ensuring unchanged command signature" {
		...
		It 'Should have the expected parameter Scope' {
			$parameter = (Get-Command Invoke-FSCPSInstallModule).Parameters['Scope']
			$parameter.Name | Should -Be 'Scope'
			$parameter.ParameterType.ToString() | Should -Be ModuleScope
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
			$parameter.ParameterSets['__AllParameterSets'].IsMandatory | Should -Be $False
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -
		__AllParameterSets -Modules -Scope
		#>
	}
