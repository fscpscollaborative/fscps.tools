...
$parentModule = 'fscps.tools'
if (-not $ModuleName) { $ModuleName = 'fscps.tools.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\fscps.tools'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\fscps.tools.psd1"
$prereqHash = @{
